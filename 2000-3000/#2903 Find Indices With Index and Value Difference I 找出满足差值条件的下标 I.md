...
```text
1. 暴力法
在每个满足 indexDifference 的条件的下标对里
检查是否满足 abs(nums[i] - nums[j]) >= valueDifference
如果满足立即返回 [i, j] 下标对
...
class Solution:
    def findIndices(self, nums: List[int], indexDifference: int, valueDifference: int) -> List[int]:
        return s[0] if (s := [[i, j] for i in range(len(nums)) for j in range(i, len(nums)) if abs(i - j) >= indexDifference and abs(nums[i] - nums[j]) >= valueDifference]) else [-1, -1]
```
